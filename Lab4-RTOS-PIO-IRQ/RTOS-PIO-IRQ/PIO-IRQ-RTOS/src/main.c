...
#define BUT_PIO_PIN1 28
#define BUT_PIO_PIN_MASK1 (1 << BUT_PIO_PIN1)
...
void but_callback(void) {
  ...
  uint32_t msg = 1;
  xQueueSendFromISR(xBut, (void *)&msg, (TickType_t) 0);
}
...
static void task_but(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (xQueueReceive(xBut, &msg, (TickType_t) 0)) {
		printf("to aqui");
      /* atualiza frequencia */
	  if (msg == 1){
		  delayTicks -= 100;
...
static void BUT_init(void) {
  ...
  pio_configure(BUT_PIO1, PIO_INPUT, BUT_PIO_PIN_MASK1, PIO_PULLUP | PIO_DEBOUNCE);

  // Configura interrupção no pino referente ao botao e associa
  // função de callback caso uma interrupção for gerada
  // a função de callback é a: but_callback()
  pio_handler_set(BUT_PIO, BUT_PIO_ID, BUT_PIO_PIN_MASK, PIO_IT_FALL_EDGE, but_callback);
  pio_handler_set(BUT_PIO1, BUT_PIO_ID1, BUT_PIO_PIN_MASK1, PIO_IT_FALL_EDGE, but_callback1);

  // Ativa interrupção e limpa primeira IRQ gerada na ativacao
  pio_enable_interrupt(BUT_PIO, BUT_PIO_PIN_MASK);
  pio_get_interrupt_status(BUT_PIO);
  pio_enable_interrupt(BUT_PIO1, BUT_PIO_PIN_MASK1);
  ...
  NVIC_SetPriority(BUT_PIO_ID1, 5); // Prioridade 5
}
...
int main(void) {
  ...
  xBut = xQueueCreate(32, sizeof(uint32_t));
  if (xBut == NULL)
	printf("falha em criar a queue \n");
